...
from deploy_mujoco.utils.math import euler_from_quat, quat_apply, wrap_to_pi, quat_rotate_inverse, quat_relative
from deploy_mujoco.utils.logger import EETrackingLogger
from deploy_mujoco.modules.ee_tracking import DLSIKController, EEGoalSampler, EEPointVisualizer
from deploy_mujoco.configs import Go2wPiperCfg
import argparse
import torch
import time
...
class Go2wPiper:
    def __init__(self, cfg: Go2wPiperCfg, actor_path=None, hist_encoder_path=None, xml_path=None):
        self.cfg = cfg
        xml_path = Path(xml_path or self.cfg.asset.file).expanduser().resolve()
        actor_path = Path(actor_path or self.cfg.asset.actor_path).expanduser().resolve()
        hist_encoder_path = Path(hist_encoder_path or self.cfg.asset.hist_encoder_path).expanduser().resolve()
        for path, label in ((xml_path, "MuJoCo XML"), (actor_path, "actor policy"), (hist_encoder_path, "history encoder")):
            if not path.exists():
                raise FileNotFoundError(f"{label} not found: {path}")

        self.model = mujoco.MjModel.from_xml_path(str(xml_path))
        self.data = mujoco.MjData(self.model)
        self.actor_policy = torch.jit.load(str(actor_path), map_location="cpu")
        self.hist_encoder_policy = torch.jit.load(str(hist_encoder_path), map_location="cpu")
...
def parse_args():
    parser = argparse.ArgumentParser(description="Deploy a Go2W-Piper policy in MuJoCo.")
    parser.add_argument("--actor-path", type=str, default=None, help="Path to traced_actor.pt.")
    parser.add_argument("--hist-encoder-path", type=str, default=None, help="Path to traced_hist_encoder.pt.")
...
def main():
    ...
    robot = Go2wPiper(
        ...
        hist_encoder_path=args.hist_encoder_path,
