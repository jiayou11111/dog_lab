...
if [[ -z "${ISAACLAB_PATH:-}" || ! -d "${ISAACLAB_PATH}/source/isaaclab" ]]; then
    ...
    echo "        export ISAACLAB_PATH=/home/user/projects/IsaacLab"
    exit 1
fi
...
if [[ -z "${CONDA_PREFIX:-}" ]]; then
    echo "[ERROR] No conda environment detected. Run 'conda activate <env>' first."
    exit 1
fi
...
# Headless defaults for local Ubuntu training. These are harmless when the
# machine has a display, and useful when running over SSH.
unset DISPLAY
export HEADLESS=1
